...
func TestAdminUsersHandler(t *testing.T) {
	t.Run("it should create an admin user", TRun(func(t *testing.T) {
		...
		newAdminUser := admin_user_handler.CreateRequestDto{
		...
		newAdminUserResponse := admin_user_handler.CreateResponseDto{}

		test_utils.Decode(t, res.Body, &newAdminUserResponse)

		expectedAdminUser := admin_user_handler.CreateResponseDto{
	...
	t.Run("it should update an user", TRun(func(t *testing.T) {
		...
		updateAdminUser := admin_user_handler.UpdateRequestDto{
		...
		updatedAdminUserNewData := admin_user_handler.UpdateRequestDto{
	...
	t.Run("it should get one by id", TRun(func(t *testing.T) {
		...
		adminUserResponse := admin_user_handler.GetOneByIdResponseDto{}

		test_utils.Decode(t, res.Body, &adminUserResponse)

		expectedAdminUser := admin_user_handler.GetOneByIdResponseDto{
			ID:        adminUser.ID,
	...
	t.Run("it should get one by email", TRun(func(t *testing.T) {
		...
		res := TMakeRequest(t, http.MethodPost, "admin/users/get-by-email", admin_user_handler.GetOneByEmailRequestDto{
			Email: adminUser.Email,
		})

		adminUserResponse := admin_user_handler.GetOneByEmailResponseDto{}

		test_utils.Decode(t, res.Body, &adminUserResponse)

		expectedAdminUser := admin_user_handler.GetOneByEmailResponseDto{
			ID:        adminUser.ID,
			FirstName: adminUser.FirstName,
			LastName:  adminUser.LastName,
			Email:     adminUser.Email,
	...
	t.Run("it should get all", TRun(func(t *testing.T) {
		...
		adminUsersResponse := []admin_user_handler.GetAllResponseDto{}
