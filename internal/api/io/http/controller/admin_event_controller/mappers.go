...
func (s *CreateResponseDto) FromDomain(p admin_event_service.CreateResponse) CreateResponseDto {
	installments := []admin_product_controller.CreateInstallmentsResponseDto{}

	for _, v := range p.ProductInstallments {
		installments = append(installments, admin_product_controller.CreateInstallmentsResponseDto{
			ID:            v.ID,
			PaymentTypeID: v.PaymentTypeID,
...
func (s *GetOneByIdResponseDto) FromDomain(p *admin_event_repository.GetOneByIdResponse) GetOneByIdResponseDto {
	...
	creditcard := []admin_product_controller.PaymentTypeInstallment{}
	paymentSlip := []admin_product_controller.PaymentTypeInstallment{}
	pix := []admin_product_controller.PaymentTypeInstallment{}

	for _, v := range p.Product.Installments.Creditcard {
		creditcard = append(creditcard, admin_product_controller.PaymentTypeInstallment{
	...
	for _, v := range p.Product.Installments.PaymentSlip {
		paymentSlip = append(paymentSlip, admin_product_controller.PaymentTypeInstallment{
			InstallmentTimeID:   v.InstallmentTimeID,
			InstallmentTimeName: v.InstallmentTimeName,
			Fee:                 v.Fee,
			Tariff:              v.Tariff,
	...
	for _, v := range p.Product.Installments.Pix {
		pix = append(pix, admin_product_controller.PaymentTypeInstallment{
			InstallmentTimeID:   v.InstallmentTimeID,
			InstallmentTimeName: v.InstallmentTimeName,
			Fee:                 v.Fee,
			Tariff:              v.Tariff,
	...
	return GetOneByIdResponseDto{
		...
		Product: GetOneByIdProductDto{
			...
			Stock: stock,
			Installments: admin_product_controller.InstallmentsResponseDto{
